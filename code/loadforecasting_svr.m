...
% Training a Support Vector Regression (SVR) model
...
legend;
title('Actual vs Predicted Electric Load (SVR)');
xlabel('Time');
ylabel('Electric Load (MW)');
hold off;

% Forecast next 48 hours (only for demonstration purposes)
n_forecast = 48;
X_forecast = X(end-n_forecast+1:end, :);
y_forecast = predict(Mdl, X_forecast);

% Generate time vector for forecast
time_forecast = data.timestamp(n_train+1:end);
time_forecast = time_forecast(end-n_forecast+1:end);

% Plot the forecast for the next 48 hours
figure;
plot(time_forecast, y_forecast, 'g', 'DisplayName', 'Forecast');
legend;
title('Support Vector Regression Forecast for Electric Load (Next 48 Hours)');
xlabel('Time');
ylabel('Electric Load (MW)');
hold off;

% Saving the trained model
save('LoadForecastModel_SVR.mat', 'Mdl');
